...
<div class="app-shell">
    ...
    <section class="panel">
        ...
        <form class="form-grid" method="get" action="${pageContext.request.contextPath}/listados">
            <label>
                ...
                <select id="tipoListado" name="tipo" required>
                    <option value="guardias" ${selectedTipo == 'guardias' ? 'selected' : ''}>Guardias</option>
                    <option value="grupos" ${selectedTipo == 'grupos' ? 'selected' : ''}>Grupos</option>
                    <option value="pertrechos" ${selectedTipo == 'pertrechos' ? 'selected' : ''}>Pertrechos</option>
                    <option value="mensajes" ${selectedTipo == 'mensajes' ? 'selected' : ''}>Mensajes</option>
                </select>
            </label>
            <label>
                Expresion de filtro
                <input id="filtroInput" type="text" name="filtro" value="${filtro}" list="filtroSugerencias" placeholder="Ej: rango:Maestre AND puntosGracia>=90 OR estadoHonor=Activo" autocomplete="off">
            </label>
            <datalist id="filtroSugerencias"></datalist>
            <div class="button-row">
                <button type="button" class="button button-secondary" data-insert-token="AND">AND</button>
                <button type="button" class="button button-secondary" data-insert-token="OR">OR</button>
                <button type="button" class="button button-secondary" data-insert-token="NOT">NOT</button>
        ...
        <div class="notice">
            ...
            <p><strong>Operadores:</strong> <code>:</code> contiene, <code>=</code> igual, <code>!=</code> distinto, <code>&gt; &gt;= &lt; &lt;=</code> numericos, <code>^=</code> empieza por, <code>$=</code> termina en, <code>~</code> regex.</p>
            <p><strong>Conectores:</strong> usa <code>AND</code> y <code>OR</code> (mayusculas o minusculas).</p>
            <p><strong>Parentesis:</strong> soportado, por ejemplo <code>rango:Maestre AND (estadoHonor=Activo OR puntosGracia&gt;90)</code>.</p>
            <p><strong>Precedencia:</strong> <code>AND</code> se evalua antes que <code>OR</code> cuando no se usan parentesis.</p>
            <p><strong>Negacion:</strong> usa <code>NOT</code> o <code>!</code>, por ejemplo <code>NOT estadoHonor=Infame</code>.</p>
            <p><strong>Busqueda global:</strong> usa <code>all:</code> para buscar en todos los campos, por ejemplo <code>all:maestre AND !all:infame</code>.</p>
        </div>
    </section>
