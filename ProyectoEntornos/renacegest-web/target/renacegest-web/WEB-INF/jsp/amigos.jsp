...
<div class="app-shell">
    <header class="page-head">
        <div>
            ...
            <p class="lead">Consulta publica para Amigos de la Guardia, con galeria social, etiquetado y valoraciones.</p>
        </div>
    ...
    <section class="panel" style="margin-top: 18px;">
        ...
        <p class="lead">Etiqueta personas en las fotos, comparte tu usuario de red y deja una valoracion para amplificar el alcance social de la asociacion.</p>
    </section>

    <section class="gallery-grid">
        <c:forEach items="${fotosGaleria}" var="foto">
            <article class="gallery-card">
                <img class="gallery-image" src="${pageContext.request.contextPath}/${foto.urlImagen}" alt="${foto.titulo}">
                <div class="gallery-body">
                    <h3>${foto.titulo}</h3>
                    <p>${foto.descripcion}</p>
                    <p><strong>Lugar:</strong> ${foto.lugarEvento}</p>
                    <p><strong>Fecha:</strong> ${foto.fechaEvento}</p>
                    <p><strong>Valoracion media:</strong> ${foto.mediaValoracion} / 5 (${foto.valoraciones.size()} valoraciones)</p>

                    <div class="tag-cloud">
                        <c:forEach items="${foto.etiquetas}" var="etiqueta">
                            <span class="member-pill">
                                ${etiqueta.nombrePersona}
                                <c:if test="${not empty etiqueta.usuarioRed}">· ${etiqueta.usuarioRed}</c:if>
                            </span>
                        </c:forEach>
                    </div>

                    <form class="form-grid" method="post" action="${pageContext.request.contextPath}/amigos">
                    ...
                    <div class="timeline">
                        <c:forEach items="${foto.valoraciones}" var="valoracion">
                            <article class="message-card">
                                ...
                                <small>
                                    <c:if test="${not empty valoracion.usuarioRed}">${valoracion.usuarioRed} · </c:if>${valoracion.fechaValoracion}
                                </small>
                            </article>
                        </c:forEach>
                    </div>
